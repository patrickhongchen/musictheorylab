...
function positionKind(position: TransitionPosition) {
...
export function BluesFretboardView({
  ...
  const paths = useMemo(() => {
    ...
    return candidates
      ...
      .filter((path, index, all) => all.findIndex(other => other.from.currentTone?.role === path.from.currentTone?.role) === index)
      .slice(0, 2)
  }, [currentChord, nextChord, positions])

  return <div className="transition-fretboard-scroll" tabIndex={0} role="region" aria-label="Chord transition fretboard, scroll horizontally on small screens">
    <svg className="transition-fretboard" viewBox="0 0 1000 320" role="img" aria-labelledby={`${titleId} ${descriptionId}`}>
      <title id={titleId}>{displayNote(currentChord.name)} to {displayNote(nextChord.name)} chord-tone movement on guitar</title>
      <desc id={descriptionId}>
        Frets {currentModel.fretStart} through {currentModel.fretEnd}. Faint dots show the home blues scale, solid green markers show current-chord tones, dark outlines show tones shared by both chords, and orange outlines show next-chord targets. {positions.filter(position => position.currentTone || position.nextTone).map(position => `String ${position.string} fret ${position.fret}: ${displayNote(position.pitchClass.name)}, ${spokenRole(position, currentChord, nextChord)}`).join('; ')}.
      </desc>
