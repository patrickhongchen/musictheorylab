...
import { SVG_DISPLAY_FONT, appendSvgText, prepareNotationSvg, useNotationRenderer, vexKey } from './notation'
...
function appendDegreeLabel(svg: SVGSVGElement, x: number, y: number, degree: string, color: string) {
  ...
  appendSvgText(svg, x, y, numeral, options)
  if (accidental) {
    appendSvgText(svg, x - 5, y, accidental, { ...options, anchor: 'end' })
  }
}
...
export default function SoloingStaffView({ model, showNextChord = false, noteFilter = 'both' }: SoloingStaffViewProps) {
  ...
  const draw = useCallback((host: HTMLDivElement) => {
    ...
    const notes = visibleTones.map(tone => {
      const note = new StaveNote({ keys: [vexKey(tone.pitch)], duration: 'q' })
    ...
    prepareNotationSvg(svg, width, height)
    ...
    notes.forEach((note, index) => {
      ...
      appendSvgText(svg, x, 151, displayNote(tone.pitch.name), {
  ...
  }, [noteFilter, showNextChord, visibleTones])
