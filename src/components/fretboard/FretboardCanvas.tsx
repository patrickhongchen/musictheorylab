...
export function FretboardGrid({
  ...
  stringStrokeIncrement = 0.2, fretStroke = '#c6c7bd',
}: FretboardGridProps) {
  return <>
    <rect x={g.nutX} y={g.boardTop} width={g.boardWidth} height={Math.max(1, g.boardHeight)} fill="#f3f1eb" />
    ...
    {g.frets.map(fret => <g key={fret}>
      {fret > 0 && <line x1={g.fretEndX(fret)} x2={g.fretEndX(fret)} y1={g.boardTop} y2={g.boardBottom} stroke={fretStroke} />}
    ...
    {[...tuning].reverse().map((note, index) => {
      ...
      return <g key={string} opacity={stringOpacity?.(string)}>
        <line x1={g.stringStartX} x2={g.endX} y1={y} y2={y} stroke={stringStroke} strokeWidth={stringStrokeWidth + index * stringStrokeIncrement} />
