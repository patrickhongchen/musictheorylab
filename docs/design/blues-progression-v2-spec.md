...
- Staff noteheads and pitch labels use the same shared role palette as the fretboard: ink for scale, green for current chord, amber for outside-scale next tones, and blue for the next-chord halo.
- Staff annotations use a restrained two-level hierarchy: the scientific pitch in Georgia above a smaller system-sans degree caption.
